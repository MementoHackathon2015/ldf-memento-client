...
HttpClient.prototype.request = function (url, method, headers, options) {
  var responseIterator = new Iterator.PassthroughIterator(true), startTime = new Date(),
      acceptHeaders = { accept: this._contentType }, self = this;
  this._logger.info('Request', url, 'with headers', JSON.stringify(headers));
  var request = {
    url: url,
    method: method || 'GET',
  ...
  .on('response', function (response) {
    ...
    self._logger.info('Response', url, response.statusCode, 'with headers', JSON.stringify(response.headers));
